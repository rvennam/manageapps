...
**附註**：Cloud Foundry Diego 架構會影響所有「{{site.data.keyword.Bluemix_notm}} 公用」地區環境。「{{site.data.keyword.Bluemix_notm}} 專用」及「{{site.data.keyword.Bluemix_notm}} 本端」環境則會在較晚的日期更新。
...
為了驗證應用程式性能，Diego 支援之前用於 DEA 的相同以埠為基礎的檢查。不過，Diego 的設計也可以具有更多的一般選項（如以 URL 為基礎的性能檢查），未來可能會啟用這些選項。
...
**附註：**緩衝區大小有限制。如果應用程式執行一段很長的時間且未重新啟動，則當您輸入 `cf logs appname --recent` 指令時可能不會顯示日誌，因為日誌緩衝區可能已被清除。因此，若要針對大型應用程式的編譯打包錯誤進行除錯，您可以在不同於 cf 指令行介面的個別指令行中輸入 `cf logs appname` 指令，以在部署應用程式時追蹤日誌。
...
對於每一個建置套件，建置套件所定義的變數都不同。如需任何其他相容建置套件，請參閱 [Buildpacks ![外部鏈結圖示](../icons/launch-glyph.svg)](https://github.com/cloudfoundry-community/cf-docs-contrib/wiki/Buildpacks){:new_window}。
